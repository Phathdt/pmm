...
@Processor(SETTLEMENT_QUEUE.SOLANA_TRANSFER.NAME)
export class SolanaTransferSettlementProcessor extends BaseProcessor {
  ...
  constructor(
    ...
    @InjectQueue(SETTLEMENT_QUEUE.SOLANA_TRANSFER.NAME) private transferSettlementQueue: Queue,
    ...
    super()
    this.logger = logger.with({ context: SolanaTransferSettlementProcessor.name })
    this.pmmId = stringToHex(this.configService.pmm.id)
  }

  @Process(SETTLEMENT_QUEUE.SOLANA_TRANSFER.JOBS.PROCESS)
  async transfer(job: Job<string>) {
    return this.executeWithTraceId(job, async (job) => {
      ...
      this.logger.log({
        message: 'Processing Solana transfer settlement retry',
        tradeId,
        retryCount,
      ...
      try {
        ...
        this.logger.log({
          message: 'Solana transfer settlement completed successfully',
          tradeId,
          paymentTxId,
          operation: 'solana_transfer_settlement',
          status: 'success',
        ...
        if (retryCount < this.MAX_RETRIES - 1) {
          ...
          await this.transferSettlementQueue.add(
            SETTLEMENT_QUEUE.SOLANA_TRANSFER.JOBS.PROCESS,
  ...
  private async transferToken(
    ...
    this.logger.log({
      message: 'Solana chain information decoded successfully',
      tradeId,
      toUserAddress,
      networkId,
      tokenAddress: toTokenAddress,
      operation: 'solana_chain_info_decode',
      timestamp: new Date().toISOString(),
    })
